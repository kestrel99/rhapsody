...
test_that("scan_params: scan overrides baseline params (not appends)", {
  # Bug: c(list(r=1), list(r=0.1)) produces two r entries; solver sees the first.
  # Fix: use [[<- assignment so the scanned value overwrites the baseline.
  ir  <- parse_model("dX/dt = r*X\nX[0]=1\nr=1\nt0=0\ntmax=1\ndt=0.01")
  out <- scan_params(ir,
    params    = list(r = 1),   # baseline matches model default
    scan_spec = list(parameter = "r", from = 0.1, to = 2, steps = 3, log_scale = FALSE)
  )
  final_X <- vapply(out$results, function(df) tail(df$X, 1L), numeric(1L))
  expect_true(all(diff(final_X) > 0))  # would be all identical values if bug present
})
